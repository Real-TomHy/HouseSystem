...
class AddInfo(object):
    ...
    def createRoot(self):
        ...
        comboxlisthous_situation["values"] = ("已出售", "未出售")
    ...
    def insertInfo(self, cus_id, cus_name, cus_price, cus_type, hous_area, hous_type, cus_cell, ren_situation,
        ...
        try:
            ...
            if not(cus_id.strip() and cus_name.strip()  and cus_type.strip()  and hous_type.strip()  and ren_situation.strip()and hous_address.strip() and hous_situation.strip()):
                raise ValueError
            if db.prepare(f"select * from SellHouseInfo where cell_id='{cus_id}'") == 0:
                sql = f"insert into SellHouseInfo values('{cus_id}', '{cus_name}', '{cus_cell}', '{cus_type}','{cus_price}','{hous_type}','{hous_area}','{ren_situation}','{hous_address}','{hous_situation}')"
                db.prepare(sql)
                db.update()
                showinfo("提示", "添加成功")
            else:
