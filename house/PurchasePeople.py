...
from house.del_purHousePeople import DelInfo
from house.addPeople import *


class PurchasePeople(object):
    def __init__(self,oldroot):
        ...
        add = AddInfo()
        Button(root, text="显示信息", command=lambda: rent.show_PurHousePeople()).place(relx=0.5, rely=0.2, width=80)
        Button(root, text="记录信息", command=lambda: add.insertPurPeopleInfo(self.id.get(),self.name.get(),self.cell.get(),self.comboxlistcus_type.get(),self.comboxlistHous_type.get(),self.area.get(),self.comboxlistren_situation.get())).place(relx=0.05, rely=0.2, width=80)
